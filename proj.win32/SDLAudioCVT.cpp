...
namespace ff
{
	...
	static AudioFilter
		SDL_HandTunedTypeCVT(AudioFormat src_fmt, AudioFormat dst_fmt)
	{
			/*
	...
	static AudioFilter
		SDL_HandTunedResampleCVT(AudioCVT * cvt, int dst_channels,
	...
	static void
		SDL_ConvertStereo(AudioCVT * cvt, AudioFormat format)
	{
			int i;
...
#undef dup_chans_1_to_2
	...
	static void
		SDL_ConvertSurround(AudioCVT * cvt, AudioFormat format)
	{
			int i;
		...
		static void SDL_ConvertStrip_2(AudioCVT * cvt, AudioFormat format)		
		{
				int i;
...
#undef strip_chans_6_to_4
	...
	*  specific data types.
	*/
	static int SDL_BuildAudioTypeCVT(AudioCVT * cvt,
		AudioFormat src_fmt, AudioFormat dst_fmt)
	{
			if (src_fmt != dst_fmt) {
				const Uint16 src_bitsize = SDL_AUDIO_BITSIZE(src_fmt);
				const Uint16 dst_bitsize = SDL_AUDIO_BITSIZE(dst_fmt);
				AudioFilter filter = SDL_HandTunedTypeCVT(src_fmt, dst_fmt);
				...
				if (filter == NULL) {
					...
					if (filter == NULL) {
						SDLog("No conversion available for these formats");
						return -1;
					}
				}
	...
	static void SDL_ConvertSurround_4(AudioCVT * cvt, AudioFormat format)
	{
			int i;
		...
		static void 
			SDL_ConvertStrip(AudioCVT * cvt, AudioFormat format)
		{
				int i;
...
#undef strip_chans_6_to_2
		...
		static void 
			SDL_ConvertMono(AudioCVT * cvt, AudioFormat format)
		{
				int i;
				Sint32 sample;
		...
		static int
			SDL_BuildAudioResampleCVT(AudioCVT * cvt, int dst_channels,
			int src_rate, int dst_rate)
		{
				if (src_rate != dst_rate) {
					AudioFilter filter = SDL_HandTunedResampleCVT(cvt, dst_channels,
						src_rate, dst_rate);
					...
					if (filter == NULL) {
						...
						if (filter == NULL) {
							SDLog("No conversion available for these rates");
							return -1;
						}
					}
		...
		AudioFormat dst_fmt, Uint8 dst_channels, int dst_rate)
	{
		...
		*/
		...
		if ((SDL_AUDIO_BITSIZE(src_fmt) > 16) && (!SDL_AUDIO_ISSIGNED(src_fmt))) {
			return SDLog("Invalid source format");
		}
		if ((SDL_AUDIO_BITSIZE(dst_fmt) > 16) && (!SDL_AUDIO_ISSIGNED(dst_fmt))) {
			return SDLog("Invalid destination format");
		}

		/* prevent possible divisions by zero, etc. */
		if ((src_channels == 0) || (dst_channels == 0)) {
			return SDLog("Source or destination channels is zero");
		}
		if ((src_rate == 0) || (dst_rate == 0)) {
			return SDLog("Source or destination rate is zero");
		}
#ifdef DEBUG_CONVERT
		...
		SDL_ConvertAudio(AudioCVT * cvt)
	{
			...
			if (cvt->buf == NULL) {
				SDLog("No buffer allocated for conversion");
				return (-1);
			}
